...
{
    #ifdef O3
    FILE* data_base  = fopen("data/intrin_xmm_O3.csv", "w");
    #elif O2
    FILE* data_base  = fopen("data/intrin_xmm_O2.csv", "w");
    #else 
    FILE* data_base  = fopen("data/intrin_xmm_O0.csv", "w");
    #endif
        ...
        for (yPix = 0;  yPix < height;  yPix++, y0 += dy)
        {
            ...
            {                   
                ...
                for (int IsDotLeft = 0; ! IsDotLeft; )
                {
                    ...
                    __m128 CmpMask2 = _mm_cmp_ps (Nres, Zero, _CMP_EQ_OS);
